...
import { FissureData, FissureResponse } from './interfaces';
...
export default class FissureService {
  ...
  public async getFissures(discordChannel: TextChannel) {
    try {
      ...
      if (Array.isArray(responseData) && response.status === 200) {
        ...
        let rolesForPinging: FissureData[] = [];
        ...
        for (const fissure of fissures) {
          ...
          if (!fissure['isStorm'] && fissure['isHard']) {
            ...
            if (fissure['missionKey'] === 'Survival' || fissure['missionKey'] === 'Excavation' || fissure['missionKey'] === 'Disruption' || fissure['missionKey'] === 'Interception') {
              ...
              //see if the mission type is already in the array
              //if not add it to the array
              const missionExistsInArray = rolesForPinging.find(role => role.missionType === '' + fissure['tier'] + fissure['missionKey']);

              if (!missionExistsInArray) {
                //add to roles for pinging array
                rolesForPinging.push({ missionType: '' + fissure['tier'] + fissure['missionKey'], fissureId: fissure['id'] });
              }
        ...
        const newFissuresForRoles: FissureData[] = await this.checkIfNotifiedAlready(rolesForPinging);
        const relicSubscriptionsForPinging: string[] = await this.getUsersToPingFromFirebase(newFissuresForRoles);
  ...
  async checkIfNotifiedAlready(rolesForPinging: FissureData[]): Promise<FissureData[] | PromiseLike<FissureData[]>> {
    //create array to return
    const newFissuresForRoles: FissureData[] = [];

    //loop through roles for pinging
    for (let i = 0; i < rolesForPinging.length; i++) {
      //get fissure data from firebase
      const db = getDatabase();
      const notificationsRef = ref(db, 'notifications/' + rolesForPinging[i].fissureId);
      const notifications = await get(notificationsRef);
      if (!notifications.exists()) {
          ...
          //if not notified, add to array to return
          newFissuresForRoles.push(rolesForPinging[i]);
        
      } else {
        console.log('notification does exist')
      }
    return newFissuresForRoles;
  }
}
  async getUsersToPingFromFirebase(rolesForPinging: FissureData[]): Promise<string[]> {
    const db = getDatabase();

    const usersToPing: Set<string> = new Set();
    for(let i =0; i < rolesForPinging.length; i++) {
      const usersRef = ref(db, 'subscriptions/' + rolesForPinging[i].missionType);
